...
form.addEventListener('submit', async (e) => {
  ...
  const resetWishVal = document.getElementById('reset-wish').value;
  const detailVal = document.getElementById('detail').value.trim();

  if (!gradeVal || !clsVal || !resetWishVal || !detailVal) {
    showMsg('학년, 반, 공장 초기화 희망여부, 상세장애내용은 필수입니다.', 'red');
    return;
  }
